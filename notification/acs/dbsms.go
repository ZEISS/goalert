...
func newDB(ctx context.Context, db *sql.DB) (*dbSMS, error) {
	...
	return &dbSMS{
		...
		lock: p(`LOCK acs_sms_callbacks IN SHARE UPDATE EXCLUSIVE MODE`),

		getInUse: p(`
			SELECT cb.code
			FROM acs_sms_callbacks cb
			WHERE
				phone_number = $1 AND (
		...
		insert: p(`
			INSERT INTO acs_sms_callbacks (phone_number, callback_id, code, alert_id, service_id)
			VALUES ($1, $2, $3, $4, $5)
		...
		lookupSvcByCode: p(`
			...
			FROM acs_sms_callbacks
			JOIN services svc ON svc.id = service_id
			WHERE phone_number = $1 AND code = $2
		`),
		lookupByCode:  p(`SELECT callback_id, alert_id, NULL FROM acs_sms_callbacks WHERE phone_number = $1 AND code = $2`),
		lookupByAlert: p(`SELECT callback_id, alert_id, NULL FROM acs_sms_callbacks WHERE phone_number = $1 AND alert_id = $2`),
...
func (db *dbSMS) insertDB(ctx context.Context, phoneNumber, callbackID string, alertID int, serviceID string) (int, error) {
	...
	defer sqlutil.Rollback(ctx, "acs: insert SMS callback", tx)
